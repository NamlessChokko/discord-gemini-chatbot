...
export function sendTypingIndicator(channel) {
    if (_instanceof(channel, TextChannel) || _instanceof(channel, DMChannel) || _instanceof(channel, NewsChannel)) {
        channel.sendTyping();
    }
}
...
export function createParts(message, media) {
    return _async_to_generator(function() {
        var parts, _iteratorNormalCompletion, _didIteratorError, _iteratorError, _iterator, _step, attachment, file, buffer, base64Data, mimeType, error, err;
        return _ts_generator(this, function(_state) {
            switch(_state.label){
                case 0:
                    parts = [];
                ...
                case 5:
                    ...
                    if (mimeType.startsWith('text/plain') && mimeType.includes('charset=')) {
