...
export async function createHistory(message, client) {
    const maxHistoryLengthConfig = config.messageCreate.generation.maxHistoryLength || 10;
...
export async function loadCommands(client) {
    ...
    for (const command of commands) {
        ...
        if ('data' in cmd && 'execute' in cmd && 'helpMessage' in cmd) {
            ...
            commandWarningLog(cmd.data?.name, 'data' in cmd, 'execute' in cmd, 'helpMessage' in cmd);
        }
    }
}
