...
export async function execute(interaction, gemini) {
    ...
    newCodeCommandLog(new Date().toLocaleString(), interaction.user.username, prompt, interaction.guild?.name || 'Direct Message');
    await interaction.reply({
        content: config.code.proccessingMessage,
        withResponse: true,
    });
    const systemInstruction = systemInstructions.code(interaction.user.globalName ||
    ...
    interaction.editReply({
        ...
        files: [
            {
                attachment: Buffer.from(parts.map((part) => part.text).join(''), 'utf-8'),
